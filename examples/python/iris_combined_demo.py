"""
Iris Dataset Demo - Combined Hexagonal DBGSOM + Vesanto Clustering
    ...
    if k > 1 and per_cluster:
        ...
        for idx, (c_id, (dominant, c_purity, c_size)) in enumerate(sorted_clusters):
            ...
            ax.annotate(
                ...
                fontsize=8, fontweight='bold', color='#475569',  # Slate-600
                ha='center', va='bottom',
...
def main():
    print("=" * 70)
    print("  Combined DBGSOM Demo: Bayesian Training + Vesanto Clustering")
    print("=" * 70)
    ...
    viz = DBGSOMVisualizer(output_dir, "iris_combined")

    # Remove empty raw_data folder created by DBGSOMVisualizer (not used in this demo)
    if viz.raw_data_path.exists() and not any(viz.raw_data_path.iterdir()):
        viz.raw_data_path.rmdir()

    # Expanded U-matrix only (not standard)
    ...
    # Labels hex (true species labels)
    ...
    # Compute purities outside try block so they're available in section 7
    ...
    print("\n" + "=" * 70)
    print("  Demo complete! Check examples/output/iris_combined/ for plots.")
    print("=" * 70)

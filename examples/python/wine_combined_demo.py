...
def main():
    print("=" * 70)
    print("  Combined DBGSOM Demo: Wine Dataset")
    print("=" * 70)

    # 1. Load Data
    print("\n1. Loading Wine dataset...")
    data_path = Path(__file__).parent.parent / "data" / "wine.csv"
    ...
    # Map class IDs to real names (UCI Wine dataset)
    # 1: Barolo, 2: Grignolino, 3: Barbera
    name_map = {1: "Barolo", 2: "Grignolino", 3: "Barbera"}
    target_names = [name_map.get(c, f"Class {c}") for c in unique_classes]
    ...
    # 4. Visualizations
    print("\n4. Visualizations...")
    viz = DBGSOMVisualizer(Path(__file__).parent.parent / "output", "wine_combined")

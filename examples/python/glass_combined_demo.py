...
def main():
    print("=" * 70)
    print("  Combined DBGSOM Demo: Glass Dataset")
    print("=" * 70)

    # 1. Load Data
    print("\n1. Loading Glass dataset...")
    data_path = Path(__file__).parent.parent / "data" / "glass.csv"
    ...
    # Map classes to 0..N if they are not 0-indexed contiguous
    class_to_idx = {c: i for i, c in enumerate(unique_classes)}
    y = np.array([class_to_idx[c] for c in y_raw])
    
    # Map class IDs to real names (UCI Glass dataset)
    # 1: Building Float, 2: Building Non-Float, 3: Vehicle Float
    # 5: Containers, 6: Tableware, 7: Headlamps
    name_map = {
        1: "Bldg Float",
        2: "Bldg Non-Float",
        3: "Veh Float",
        4: "Veh Non-Float",  # Not typically present
        5: "Containers",
        6: "Tableware",
        7: "Headlamps"
    }
    target_names = [name_map.get(c, f"Type {c}") for c in unique_classes]
    ...
    print("\n2. Training DBGSOM...")
    som = SEDBGSOM(
        lambda_=1, max_neurons=100, n_iter=150, init_size=(2, 2),
        random_state=19,
    )
    som.fit(df)
    ...
    # 4. Visualizations
    print("\n4. Visualizations...")
    viz = DBGSOMVisualizer(Path(__file__).parent.parent / "output", "glass_combined")

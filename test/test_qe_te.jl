@testset "QE and TE Correctness" begin
    ...
    # ========================================================================
    # Bayesian Optimization Integration Tests
    # ========================================================================
    @testset "Bayesian Optimization - QE/TE Integration" begin

        # ----------------------------------------------------------------
        # 1. best_model produces a valid QE (best_model is retrained
        #    separately so QE may differ from trial QE due to random init)
        # ----------------------------------------------------------------
        @testset "best_model QE is finite and positive" begin
            Random.seed!(42)
            # Simple 2D blob data
            X = randn(Float64, 2, 40)
            ...
            if result.best_model !== nothing
                recomputed_qe = compute_quantization_error(result.best_model, X)
                # best_model is retrained with same params but different random
                # init, so QE won't match exactly. Verify it is valid and
                # in the same order of magnitude.
                @test isfinite(recomputed_qe)
                @test recomputed_qe > 0.0
                @test isfinite(result.best_qe)
                @test result.best_qe > 0.0
            else
                @warn "No feasible solution found in bayesian QE test (non-determinism possible)"
            end
        end

        # ----------------------------------------------------------------
        # 2. best_model TE is valid and in [0,1]
        # ----------------------------------------------------------------
        @testset "best_model TE is valid and in [0,1]" begin
            Random.seed!(123)
            X = randn(Float64, 3, 30)
            ...
            if result.best_model !== nothing
                recomputed_te = compute_topographic_error(result.best_model, X)
                # best_model is retrained so TE may differ from best_te,
                # but it should be a valid value in [0, 1]
                @test 0.0 <= recomputed_te <= 1.0
                @test 0.0 <= result.best_te <= 1.0
            else
                @warn "No feasible solution found in bayesian TE test (non-determinism possible)"
            end
        end

        # ----------------------------------------------------------------
        # 3. Feasibility: best_te <= te_constraint if feasible solution found
        # ----------------------------------------------------------------
        @testset "feasibility: best_te <= te_constraint" begin
            Random.seed!(456)
            X = randn(Float64, 2, 50)
            ...
            if result.best_model !== nothing
                @test result.best_te <= te_constraint
            end
        end

        # ----------------------------------------------------------------
        # 4. Score computation: feasible => score == qe,
        #    infeasible => score == qe + 1000*(te - constraint)
        # ----------------------------------------------------------------
        @testset "trial score computation is correct" begin
            Random.seed!(789)
            X = randn(Float64, 2, 30)
            ...
            for trial in result.all_trials
                ...
                if feasible
                    @test te <= te_constraint
                    @test score ≈ qe atol=1e-10
                else
                    @test te > te_constraint
                    expected_score = qe + 1000.0 * (te - te_constraint)
                    @test score ≈ expected_score atol=1e-10
                end
            end
        end

        # ----------------------------------------------------------------
        # 5. StatisticalError method works in bayesian_optimize
        # ----------------------------------------------------------------
        @testset "bayesian_optimize works with StatisticalError method" begin
            Random.seed!(999)
            X = randn(Float64, 2, 30)
            ...
            # Should complete without error and have trial results
            @test length(result.all_trials) == 3

            # Verify score correctness for StatisticalError trials too
            for trial in result.all_trials
                if trial[:qe] < Inf
                    if trial[:feasible]
                        @test trial[:score] ≈ trial[:qe] atol=1e-10
                    else
                        expected = trial[:qe] + 1000.0 * (trial[:te] - 0.5)
                        @test trial[:score] ≈ expected atol=1e-10
                    end
                end
            end
        end
